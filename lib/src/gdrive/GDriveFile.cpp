...
bool GDriveFile::pollChange(bool longPoll) {
    ...
    try {
        const auto responseJson =
            this->request->GET(this->_baseUrl + "/files/" + this->resourceId, {{P::QUERY_PARAMS, {{"fields", "etag"}}}})
                .json();
...
std::string GDriveFile::read() const {
    ...
    try {
        const auto responseJson =
            this->request
                ->GET(this->_baseUrl + "/files/" + this->resourceId, {{P::QUERY_PARAMS, {{"fields", "downloadUrl"}}}})
                .json();
