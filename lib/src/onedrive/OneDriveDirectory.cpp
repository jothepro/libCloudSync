    ...
    try {
        const auto responseJson = this->request
                                      ->POST(
                                          ...
                                          json{{"name", newDirectoryName}, {"folder", json::object()}}.dump())
                                      .json();
...
std::shared_ptr<Resource> OneDriveDirectory::parseDriveItem(const json &value, const std::string &expectedType) const {
