...
void OneDriveDirectory::remove() {
    try {
        if (this->path() != "/") {
            m_request->DELETE(m_base_url + ":" + this->path())
                    ->token_auth(m_credentials->get_current_access_token())
                    ->send();
        } else {
...
OneDriveDirectory::parseDriveItem(const json &value, const std::string &expectedType) const {
    ...
    if (value.find("root") != value.end()) {
        ...
        if (value.find("file") != value.end() && (expectedType.empty() || expectedType == "file")) {
            ...
            resource = std::make_shared<OneDriveFile>(m_base_url, resourcePath + name, m_credentials, m_request, name,
                                                      etag);
        } else if (value.find("folder") != value.end() && (expectedType.empty() || expectedType == "folder")) {
            resource = std::make_shared<OneDriveDirectory>(m_base_url, resourcePath + name, m_credentials, m_request,
                                                           name);
        } else {
            throw exceptions::resource::NoSuchResource("unexpected resource type");
        }
    }
    return resource;
}
